...
export default function AssignmentEditor() {
    ...
    return (!assignment && !creatingNewAssignment) ? <div>No assignment found!</div> : (
        <div id="wd-assignments-editor">
            ...
            <textarea id="wd-description" className="form-control mb-2" cols={50} rows={8} value={description} onChange={(e) => setDescription(e.target.value)}></textarea>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    if (e.target.checked) {
                                        ...
                                        setOnlineEntryOptions(onlineEntryOptions.filter(o => o !== option));
                                    }
                                }}
                            />
                            <label htmlFor={`wd-${option}`} className="form-check-label">{option}</label>
                        </div>
                    ))}
                </div>
            </div>
            <div className="row">
                ...
                <div className="col-7 border rounded-2 p-3">
                    <div className="mb-2">
                        ...
                        <input id="wd-assign-to" type="text" value={assignTo} onChange={(e) => setAssignTo(e.target.value)} className="form-control" />
                    </div>
                    <div className="mb-2">
                    ...
                    <div className="row mb-2">
                        <div className="col-6">
                            ...
                            <input id="wd-available-from" type="date" value={releaseDate} onChange={(e) => setReleaseDate(e.target.value)} className="form-control" />
                        </div>
                        <div className="col-6">
                            <label htmlFor="wd-available-until" className="form-label"><b>Until</b></label>
                            <input id="wd-available-until" type="date" value={availableUntil} onChange={(e) => setAvailableUntil(e.target.value)} className="form-control" />
                        </div>
                    </div>
                </div>
            </div>
            <hr />
            <button onClick={saveAssignment} className="btn btn-danger float-end">Save</button>
